...
import { AnswerKeyEntry, Exam, AdminProfile, AdminTab, FetchedAnswerKeySummary, AnnotatorInfo, AnalyticsData, UserExamScoreMetrics } from '../../types';
...
const convertToCSV = (data: AnnotatorInfo[], columnsToInclude: {key: keyof AnnotatorInfo | string, header: string, isExamSpecific?: boolean, examCode?: string, metricKey?: keyof UserExamScoreMetrics}[]) => {
  ...
  const rows = data.map(row => {
    return columnsToInclude.map(col => {
      ...
      if (col.isExamSpecific && col.examCode && col.metricKey) {
        value = row.per_exam_scores?.[col.examCode]?.[col.metricKey];
      } else {
      ...
      if (value === null || value === undefined) {
        value = '';
      } else if (typeof value === 'number' && (col.key === 'overall_score_percentage' || col.metricKey === 'score_percentage')) {
...
const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({ adminId, onAdminLogout }) => {
  ...
  const [editingAnswerKey, setEditingAnswerKey] = useState<AnswerKeyEntry | null>(null);
  const [activeAnswerKeyExamCode, setActiveAnswerKeyExamCode] = useState<string>(EXAMS_DATA.length > 0 ? EXAMS_DATA[0].id : '');

  
  const [allAnnotators, setAllAnnotators] = useState<AnnotatorInfo[]>([]);
  const [isLoadingAnnotators, setIsLoadingAnnotators] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchAdminProfile = async () => {
        ...
        if (user) {
            setCurrentAdminProfile({ id: user.id, email: user.email || '', role: 'admin' });
        } else {
  ...
  const fetchAnswerKeySummaries = useCallback(async () => {
    ...
    try {
      ...
      const summariesWithUrls = data.map((summary: any) => {
        const { data: urlData } = supabase.storage.from(STORAGE_BUCKET_NAME).getPublicUrl(summary.storage_path);
        return {
  ...
  const fetchAnnotators = useCallback(async () => {
    ...
    try {
        ...
        if (error) {
            ...
            alert(`Failed to load annotators: ${error.message}. Ensure RPC 'get_all_annotators_with_per_exam_scores' is created and returns the expected columns.`);
            setAllAnnotators([]);
        } else {
            setAllAnnotators((data || []).map((item: any) => {
                ...
                EXAMS_DATA.forEach(exam => {
                    ...
                    const key_ks = item[`${examCode}_key_ks`] || 0;
                    const effective_ks = item[`${examCode}_effective_ks`] || 0;
                    per_exam_scores[examCode] = {
                        ...
                        key_ks: key_ks,
                        score_percentage: key_ks > 0 ? parseFloat(((effective_ks / key_ks) * 100).toFixed(1)) : 0,
                    };
                });
                ...
                return {
                    ...
                    sum_total_effective_user_keystrokes_overall: item.sum_total_effective_user_keystrokes_overall || 0,
                    sum_total_answer_key_keystrokes_overall: item.sum_total_answer_key_keystrokes_overall || 0,
                    overall_score_percentage: (item.sum_total_answer_key_keystrokes_overall && item.sum_total_answer_key_keystrokes_overall > 0)
                        ? parseFloat(((item.sum_total_effective_user_keystrokes_overall / item.sum_total_answer_key_keystrokes_overall) * 100).toFixed(1))
                        : 0,
  ...
  const fetchAnalyticsData = useCallback(async () => {
    ...
    try {
        const [
            ...
            supabase.from('annotation_rows').select('*', { count: 'exact', head: true }).eq('is_submitted', true),
  ...
  }, [activeTab, showAnswerKeyForm, fetchAnswerKeySummaries, fetchAnnotators, fetchAnalyticsData]);


  const getExamDatabaseId = async (examCode: string): Promise<number | null> => {
  ...
  const handleSaveAnswerKey = useCallback(async (keyData: AnswerKeyEntry) => {
    ...
    try {
        ...
        const examDbId = keyData.dbExamId || await getExamDatabaseId(keyData.examId);
        if (!examDbId) {
            throw new Error(`Could not find database ID for exam code: ${keyData.examId}`);
        }
        ...
        if (storagePath && (!imageDbId || keyData.imageFile)) {
            ...
            if (imageDbId && keyData.imageFile) {
                 const { data, error: updateImageError } = await supabase
                    .from('images').update(imageRecord).eq('id', imageDbId).select('id').single();
                if (updateImageError) throw updateImageError;
                imageDbId = data?.id;
            } else if (!imageDbId) {
                const { data, error: insertImageError } = await supabase
                    .from('images').insert(imageRecord).select('id').single();
        ...
        if (answerRowsToInsert.length > 0) {
            const { error: insertAnswersError } = await supabase.from('answer_key_rows').insert(answerRowsToInsert);
  ...
  const handleDeleteAnswerKey = async (dbImageId: number) => {
    ...
    setIsLoadingAnswerKeys(true);
    try {
        const { error } = await supabase.from('answer_key_rows').delete().eq('image_id', dbImageId);
        if (error) throw error;
  ...
  const filteredAnnotators = useMemo(() => {
    ...
    return allAnnotators.filter(annotator =>
      annotator.liftapp_user_id.toLowerCase().includes(annotatorSearchTerm.toLowerCase())
    );
  }, [allAnnotators, annotatorSearchTerm]);

  const handleExportAnnotatorsToCSV = () => {
    const columnsToExport: {key: keyof AnnotatorInfo | string, header: string, isExamSpecific?: boolean, examCode?: string, metricKey?: keyof UserExamScoreMetrics}[] = [
        { key: 'id', header: 'DB ID' },
        ...
        { key: 'sum_total_effective_user_keystrokes_overall', header: 'Overall Effective KS' },
        { key: 'sum_total_answer_key_keystrokes_overall', header: 'Overall Total Key KS' },
    ...
    EXAMS_DATA.forEach(exam => {
        ...
        columnsToExport.push({ key: `${exam.id}_effective_ks`, header: `${exam.name} Effective KS`, isExamSpecific: true, examCode: exam.id, metricKey: 'effective_ks' });
        columnsToExport.push({ key: `${exam.id}_key_ks`, header: `${exam.name} Total Key KS`, isExamSpecific: true, examCode: exam.id, metricKey: 'key_ks' });
  ...
  const filteredAnswerKeys = useMemo(() => {
    ...
    return fetchedAnswerKeys.filter(key => key.examCode === activeAnswerKeyExamCode);
    ...
    switch(activeTab) {
        ...
        return (
          <div className="p-6 bg-slate-50 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-slate-700">Manage Exam Answer Keys</h3>
                {!showAnswerKeyForm && (
                     <button 
            ...
            {showAnswerKeyForm ? (
              ...
              <>
                <div className="mb-4 border-b border-slate-300">
                  <nav className="flex flex-wrap -mb-px space-x-1 sm:space-x-2" aria-label="Exam Types">
                    {EXAMS_DATA.map(exam => (
                      <button
                        key={exam.id}
                ...
                <div>
                  {isLoadingAnswerKeys && <p className="text-slate-500 italic">Loading answer keys for {EXAMS_DATA.find(e=>e.id === activeAnswerKeyExamCode)?.name || 'selected exam'}...</p>}
                  {!isLoadingAnswerKeys && filteredAnswerKeys.length === 0 && (
                      <p className="text-slate-500 italic">No answer keys found for {EXAMS_DATA.find(e=>e.id === activeAnswerKeyExamCode)?.name || 'selected exam'}.</p>
                  )}
                  {!isLoadingAnswerKeys && filteredAnswerKeys.length > 0 && (
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-sm text-left text-slate-600">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-200">
                          <tr>
                            <th scope="col" className="px-4 py-3">Image Preview</th>
                            <th scope="col" className="px-4 py-3">Image Ref</th>
                            <th scope="col" className="px-4 py-3">Answer Rows</th>
                            <th scope="col" className="px-4 py-3">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredAnswerKeys.map(keySummary => (
                              <tr key={keySummary.dbImageId} className="bg-white border-b hover:bg-slate-50">
                                <td className="px-4 py-3">
                                  {keySummary.imageUrl && <img src={keySummary.imageUrl} alt={keySummary.originalFilename || keySummary.storagePath} className="h-10 w-auto object-contain rounded"/>}
                                </td>
                                <td className="px-4 py-3">{keySummary.originalFilename || keySummary.storagePath}</td>
                                <td className="px-4 py-3">{keySummary.answerRowCount}</td>
                                <td className="px-4 py-3">
                                  <button
                                    onClick={() => handleEditAnswerKey(keySummary)}
      ...
      case 'ANNOTATORS':
        return (
          <div className="p-6 bg-slate-50 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-slate-700">Annotator Management & Scores</h3>
                 <button 
                    onClick={handleExportAnnotatorsToCSV}
                    className="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors"
                    disabled={isLoadingAnnotators || filteredAnnotators.length === 0}
                >
                    Export to CSV
                </button>
            </div>
            ...
            {isLoadingAnnotators && <p className="text-slate-500 italic">Loading annotators...</p>}
            {!isLoadingAnnotators && filteredAnnotators.length === 0 && (
                <p className="text-slate-500 italic">No annotators found{annotatorSearchTerm ? ' matching your search' : ''}.</p>
            )}
            {!isLoadingAnnotators && filteredAnnotators.length > 0 && (
              <div className="overflow-x-auto">
                <table className="min-w-full text-xs text-left text-slate-600 whitespace-nowrap">
                  <thead className="text-xs text-slate-700 uppercase bg-slate-200">
                    <tr>
                      ...
                      <th scope="col" className="px-3 py-3 text-center">Overall Eff. KS</th>
                      <th scope="col" className="px-3 py-3 text-center">Overall Key KS</th>
                      ...
                      {EXAMS_DATA.map(exam => (
                        <React.Fragment key={exam.id}>
                          ...
                          <th scope="col" className="px-3 py-3 text-center">{exam.name} Eff. KS</th>
                          <th scope="col" className="px-3 py-3 text-center">{exam.name} Key KS</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredAnnotators.map(annotator => (
                      <tr key={annotator.id} className="bg-white hover:bg-slate-50">
                        ...
                        <td className="px-3 py-3 text-center text-green-600 font-semibold">{annotator.sum_total_effective_user_keystrokes_overall ?? 'N/A'}</td>
                        <td className="px-3 py-3 text-center">{annotator.sum_total_answer_key_keystrokes_overall ?? 'N/A'}</td>
                        <td className="px-3 py-3 text-center">
                            {annotator.overall_score_percentage !== undefined && annotator.overall_score_percentage !== null ? (
                                <span className={`font-bold px-2 py-1 rounded-full text-xs ${
                                    annotator.overall_score_percentage >= 75 ? 'bg-green-100 text-green-700' :
                                    annotator.overall_score_percentage >= 50 ? 'bg-yellow-100 text-yellow-700' :
                        ...
                        {EXAMS_DATA.map(exam => {
                            ...
                            return (
                                <React.Fragment key={exam.id}>
                                    ...
                                    <td className="px-3 py-3 text-center text-green-600 font-semibold">{examScores?.effective_ks ?? 0}</td>
                                    <td className="px-3 py-3 text-center">{examScores?.key_ks ?? 0}</td>
                                    <td className="px-3 py-3 text-center">
                                    {examScores?.score_percentage !== undefined && examScores?.score_percentage !== null && examScores.key_ks! > 0 ? (
                                        <span className={`font-bold px-2 py-1 rounded-full text-xs ${
        ...
        case 'ANALYTICS':
            return (
              <div className="p-6 bg-slate-50 rounded-lg shadow">
                <h3 className="text-xl font-semibold text-slate-700 mb-6">Application Analytics</h3>
                {isLoadingAnalytics && <p className="text-slate-500 italic">Loading analytics data...</p>}
                {!isLoadingAnalytics && !analyticsData && <p className="text-slate-500 italic">Analytics data could not be loaded.</p>}
                {!isLoadingAnalytics && analyticsData && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <AnalyticsCard title="Total Annotators" value={analyticsData.totalAnnotators.toString()} />
                    <AnalyticsCard title="Total Exams Configured" value={analyticsData.totalExams.toString()} />
                    <AnalyticsCard title="Total Images in System" value={analyticsData.totalImages.toString()} />
                    <AnalyticsCard title="Total Submitted Annotation Rows" value={analyticsData.totalSubmittedAnnotationRows.toString()} />
                  
                    {analyticsData.submissionsPerExam.length > 0 && (
                        <div className="md:col-span-2 lg:col-span-3 p-4 bg-white rounded-lg shadow">
                            <h4 className="text-md font-semibold text-slate-600 mb-3">Submissions per Exam</h4>
                            <ul className="space-y-2 text-sm">
                            {analyticsData.submissionsPerExam.map(examStat => (
                                <li key={examStat.name} className="flex justify-between items-center p-2 bg-slate-100 rounded">
                                <span>{examStat.name}</span>
                                <span className="font-semibold">{examStat.submission_count}</span>
                                </li>
                            ))}
                            </ul>
                        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-10 h-10 text-red-400 mr-3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
              </svg>
              <h1 className="text-2xl font-semibold">LiftApp Admin Dashboard</h1>
            </div>
            <div className="flex items-center space-x-4">
              <span className="text-sm hidden sm:block" aria-label={`Logged in as ${adminId}`}>
                Welcome, <span className="font-medium">{currentAdminProfile?.email || adminId}</span>
              </span>
              <button
